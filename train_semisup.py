...
class unsup_RANZCRDataset(Dataset):
    def __init__(self, data_dir, weak_transform=None, strong_transform=None):
    ...
    def __getitem__(self, idx):
        ...
        w_image = self.weak_transform(image=img)
        w_image = torch.from_numpy(w_image["image"].transpose(2, 0, 1))

        s_image = self.strong_transform(image=img)
        s_image = torch.from_numpy(s_image["image"].transpose(2, 0, 1))
        
        return w_image, s_image
...
class RANZCRDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == 'fit':
            ...
            self.unsup_train_dataset = unsup_RANZCRDataset(self.conf.nih_data_dir, weak_transform=valid_transform, strong_transform=train_transform)
    ...
    def train_dataloader(self):
        return [DataLoader(self.train_dataset, batch_size=self.conf.batch_size, num_workers=4, shuffle=True, pin_memory=True, drop_last=True),
                DataLoader(self.unsup_train_dataset, batch_size=self.conf.unsup_batch_size, num_workers=4, shuffle=True, pin_memory=True, drop_last=True)]
...
class LitSystem(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log('train_unsup_loss', unsup_loss, on_epoch=True)
