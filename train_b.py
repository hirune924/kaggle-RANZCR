...
class RANZCRDataset(Dataset):
    def __init__(self, dataframe, data_dir, target=None, transform=None, train=True):
        ...
        self.NGT = ['NGT - Abnormal', 'NGT - Borderline', 'NGT - Incompletely Imaged', 'NGT - Normal']
        self.CVC = ['CVC - Abnormal', 'CVC - Borderline', 'CVC - Normal']
        self.SGC = ['Swan Ganz Catheter Present']
        
        if target == 'ETT':
            self.target = self.ETT
        elif target == 'NGT':
            self.target = self.NGT
        elif target == 'CVC':
            self.target = self.CVC
    ...
    def setup(self, stage=None):
        if stage == 'fit':
            ...
            train_transform = A.Compose([
                        A.RandomResizedCrop(height=self.conf.image_size, width=self.conf.image_size, scale=(0.9, 1.0), ratio=(1, 1), interpolation=1, always_apply=False, p=1.0),
...
class LitSystem(pl.LightningModule):
    def __init__(self, conf):
        ...
        if conf.target == 'ETT':
            ...
            num_classes = 3
        self.model = timm.create_model(model_name=self.hparams.model_name, num_classes=num_classes, pretrained=True, in_chans=3)
